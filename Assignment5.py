...
def generate_batch_cbow(batch_size, num_skips, skip_window):
    ...
    for i in range(batch_size):
        # mask = [1] * span
        # mask[skip_window] = 0
        # lol=np.array(compress(buffer, mask)) # all surrounding words
        temp_buffer= buffer.copy()
        del temp_buffer[skip_window]
        batch[i, :] = temp_buffer
...
skipgram_mode=0
cbow_mode=1
training_mode=skipgram_mode

if training_mode == skipgram_mode:
    print('\nIn SkipGram : ')
    ...
    for num_skips, skip_window in [(2, 1), (4, 2)]:
        ...
        for list in batch:
            print("{", end="")
            for elem in list:
                print(reverse_dictionary[elem], end="")
                if (elem != list[num_skips - 1]):
                    print(", ", end="")
            print("}   ", end="")
        print("]")
        print('    labels:', [reverse_dictionary[li] for li in labels.reshape(8)])
